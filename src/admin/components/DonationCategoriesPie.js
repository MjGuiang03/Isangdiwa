...
export default function DonationCategoriesPie({ categoryBreakdown = {} }) {
  ...
  return (
    <div className="bg-white dark:bg-[#1E2130] rounded-2xl border border-slate-200 dark:border-white/10 shadow-sm p-6 flex flex-col h-full justify-between">
      {/* Header */}
      <div className="flex items-center justify-between mb-4 shrink-0">
        <div>
          <h3 className="m-0 font-inter text-base font-bold text-slate-900 dark:text-white tracking-tight">Donation Categories</h3>
      ...
      <div className="flex flex-col flex-1 justify-between gap-3 my-1">
        {sortedDonationData.map((item, idx) => {
          const shortName = item.name.replace('Department', 'Dept');
          return (
            <div
              key={idx}
              className="group flex flex-col justify-center px-4 py-3 rounded-xl bg-slate-50/70 dark:bg-white/[0.02] border border-slate-100 dark:border-white/5 hover:border-blue-200 dark:hover:border-blue-500/30 transition-all duration-200"
            >
