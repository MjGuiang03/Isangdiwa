...
Respond with a JSON object only:
{
  ...
  "reason": "brief explanation in 1 sentence"
    ...
    // Handle rate limiting — allow the image through with graceful fallback
    if (aiResponse === '__RATE_LIMITED__') {
      console.warn('[Loan Receipt Validation] Rate limited — allowing image through');
      return res.json({
        success: true,
        isReceipt: true,
        confidence: 0,
    ...
    if (!aiResponse) {
      console.warn('[Loan Receipt Validation] Gemini returned null — allowing image through');
      return res.json({
        success: true,
        isReceipt: true,
        confidence: 0,
    ...
    try {
      ...
      console.error('[Loan Receipt Validation] Failed to parse AI response:', aiResponse);
      return res.json({
        success: true,
        isReceipt: true,
        confidence: 0,
    ...
    console.error('[Loan Receipt Validation Error]:', err.message);
    return res.json({
      success: true,
      isReceipt: true,
      confidence: 0,
